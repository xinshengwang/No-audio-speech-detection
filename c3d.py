...
from utils.Dataloader import c3d_data,Acl_data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modality',type=str,default='c3d')
    ...
    parser.add_argument('--modal_type',type=str,default='C3D')
    ...
    parser.add_argument('--result_file',default='final_eval.text',type=str)
    ...
    for split_num in range(6):
        ...
        if args.split == 'train':
            dataset = c3d_data(args.data_path, 'train',split_num=args.split_num, threshold=args.threshold)
            dataset_val = c3d_data(args.data_path,'val',split_num=args.split_num,threshold=args.threshold)
            ...
            dataset_test = c3d_data(args.data_path,'test')
            test_loader = torch.utils.data.DataLoader(
                dataset_val, batch_size=args.batch_size,
                drop_last=False,shuffle=False,num_workers=args.workers,worker_init_fn=worker_init_fn) 


        model = c3dModels.RNN(args)

        if args.split == 'train':
            train_unimodal(model,train_loader,val_loader,args)
    """
    args.split_num = 0
    dataset = c3d_data(args.data_path, 'trainval',split_num=args.split_num, threshold=args.threshold)
    dataset_val = c3d_data(args.data_path,'val',split_num=args.split_num,threshold=args.threshold)
    train_loader = torch.utils.data.DataLoader(
        dataset, batch_size=args.batch_size,
        drop_last=False,shuffle=True,num_workers=args.workers,worker_init_fn=worker_init_fn,)
    ...
    if args.split == 'train':
        train_unimodal(model,train_loader,val_loader,test_loader,args)